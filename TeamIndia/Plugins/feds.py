...
def is_user_fed_admin(fed_id, user_id):
    fed_admins = all_fed_users(fed_id)
...
async def new(event):
 ...
 fed_name = name
 x = new_fed(event.sender_id, fed_name, fed_id)
...
async def cgname(event):
 ...
 for f in fedowner:
            fed_id = f["fed_id"]
            name = f["fed"]["fname"]
 rename_fed(fed_id, user_id, newname)
...
async def jf(event):
 ...
 getfed = search_fed_by_id(args)
 name = getfed["fname"]
 if not getfed:
  return await event.reply("This FedID does not refer to an existing federation.")
 fed_id = get_fed_id(event.chat_id)
 if fed_id:
    chat_leave_fed(event.chat_id)
 x = chat_join_fed(args, event.chat.title, event.chat_id)
...
async def p(event):
 ...
 if user_id == event.sender_id:
    return await event.reply("Yeah well you are the owner!")
 fban, fbanreason, fbantime = get_fban_user(fed_id, int(args.id))
 ...
 if fban:
  if fbanreason != '':
   ...
   return await event.reply(f"User {fname} is fbanned in {name}. You should unfban them before promoting.")
 getuser = search_user_in_fed(fed_id, user_id)
 if getuser:
...
async def smex_fed(event):
  ...
  user = user.strip()
  owner = owner.strip()
  fedowner = get_user_owner_fed_full(int(owner))
  for f in fedowner:
            fed_id = f["fed_id"]
            name = f["fed"]["fname"]
  ...
  res = user_join_fed(fed_id, int(user))
  if res:
...
async def info(event):
 ...
 if info:
  ...
  getfban = get_all_fban_users(fed_id)
  getfchat = all_fed_chats(fed_id)
  FEDADMIN = all_fed_users(fed_id)
  ...
  try:
     subs = get_subscriber(fed_id)
  except:
     subs = []
  caption += f"Number of subscribed feds: `{len(subs)}`"
  try:
    getmy = get_mysubs(fed_id)
  except:
    getmy = []
  if len(getmy) == 0:
...
async def smex_fed(event):
  ...
  try:
        ...
        members = all_fed_members(fed_id)
        for x in members:
          try:
...
async def _(event):
    ...
    fban, fbanreason, fbantime = get_fban_user(fed_id, int(r_sender_id))
    if fban:
    ...
    if not fban:
       ...
       sax += f"**Reason:** {reason}{shrunk}"
            ...
            rec = mal.replace("-", "")
            fed_name = info["fname"]
            temp = un_fban_user(fed_id, fban_user_id)
            if not temp:
    ...
    await tbot.send_message(
                event.chat_id,
                sax)
    getfednotif = user_feds_report(info["owner"])
    if getfednotif:
      if int(info["owner"]) != int(chat):
         await tbot.send_message(
                int(info["owner"]),
                sax)
    get_fedlog = get_fed_log(fed_id)
    if get_fedlog:
        if int(get_fedlog) != int(chat):
           await tbot.send_message(
                int(get_fedlog),
                sax)
    fed_chats = list(all_fed_chats(fed_id))
    if len(fed_chats) != 0:
        for fedschat in fed_chats:
                try:
                    ...
                    pass
    subscriber = list(get_subscriber(fed_id))
    if len(subscriber) != 0:
           for fedsid in subscriber:
                ...
                int(rec),
                 ...
                 all_fedschat = all_fed_chats(fedsid)
...
async def sub(event):
 ...
 subfed = subs_fed(args, fed_id)
 addsub = add_sub(fed_id, args)
...
async def unsub(event):
 ...
 if not getfed:
    return await event.reply("This FedID does not refer to an existing federation.")
 sname = getfed["fname"]
 remsub = rem_sub(fed_id, args)
 unsubfed = unsubs_fed(args, fed_id)
...
async def fstat(event):
 ...
 if args:
  if len(args) >= 16:
    ...
    fban, fbanreason, fbantime = get_fban_user(args, int(user_id))
    if not fban:
 ...
 uname, fbanlist = get_user_fbanlist(str(user_id))
 if len(fbanlist) == 0:
...
async def fex(event):
 ...
 try:
            for users in getfban:
                getuserinfo = get_all_fban_users_target(fed_id, users)
                json_parser = {
                    "user_id": users,
...
async def ft(event):
 ...
 fedora = get_user_owner_fed_full(user_id)
 try:
...
async def smex_fed(event):
  ...
  user = user.strip()
  cname = owner.strip()
  fedowner = get_user_owner_fed_full(int(cname))
  for f in fedowner:
          fed_id = f["fed_id"]
          name = f["fed"]["fname"]
  rt = await tbot(GetFullUserRequest(int(user)))
  ...
  res = tr_fed(fed_id, int(user))
  if not res:
    return await event.edit("Fed transfer failed!")
  if res:
    ses = user_join_fed(fed_id, int(cname))
    user_demote_fed(fed_id, int(user))
  ...
  getfednotif = user_feds_report(int(user))
...
__mod_name__ = "Fedration"

__help__ = """
management. It's all fun and games, until you start getting spammers in, and you need to ban them. Then you need to start banning more, and more, and it gets painful.
