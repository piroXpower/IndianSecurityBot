...
import TeamIndia.Plugins.sql.global_bans_sql as sql
from TeamIndia import (DEV_USERS, EVENT_LOGS, OWNER_ID, STRICT_GBAN, DRAGONS,
...
from TeamIndia.Plugins.helper_funcs.misc import send_to_list
from TeamIndia.Plugins.sql.users_sql import get_all_chats
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of the Alicia Team!\nI can't act against our own."
        )
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            "Issei! Show em power of the sudo!"
        )
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            "Trying to gban a Alicia servant?, you got a death wish or something?")
        return
    ...
    message.reply_text("Strike on it! This user is going to globally banned!")
    ...
    if gban_time > 60:
        ...
        message.reply_text(f"Master!.. <code>{mention_html(user.id, user.first_name)}</code> Gbanned This User <code>{mention_html(user_chat.id, user_chat.first_name)}</code>\n<b>Chats affected:</b> <code>{gbanned_chats}</code>\n #Gbanned.", parse_mode=ParseMode.HTML)
