    ...
    if len(pairs) > 10:
        pairs = pairs[modulo_page * 10:10 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("🔺", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("Back", callback_data="india_back"),
             EqInlineKeyboardButton("🔻", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [
                  [
                    EqInlineKeyboardButton("╰✰ Cʜᴀɴɴᴇʟ", url="t.me/IndianUpdateChannel"),                    
                    EqInlineKeyboardButton("Sᴜᴘᴘᴏʀᴛ ✰╮", url="t.me/IndianSupportGroup")
                  ], 
                  [
                    EqInlineKeyboardButton("✗ Bᴀᴄᴋ ✗", callback_data="india_back")
                  ]
                 ]
